...
| --- | --- | --- |
| Ingestion assurance | `customer_ingestion_reconciliation`, `order_ingestion_reconciliation`, `platform_ingestion_reconciliation`, and `platform_ingestion_volume_reconciliation` | Compare landed order and customer extracts with their normalized outputs. The order review keeps one business interface while dispatching only the warehouse-specific aggregation syntax; the volume review applies one feed inventory and an explicit minute-level review policy across both platform inputs, then reports each raw-to-staged row-count delta. |
