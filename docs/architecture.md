...
| --- | --- | --- |
| Ingestion assurance | `customer_ingestion_reconciliation`, `order_ingestion_reconciliation`, `platform_ingestion_reconciliation`, `platform_ingestion_volume_reconciliation`, and `platform_ingestion_scheduled_volume_reconciliation` | Compare landed order and customer extracts with their normalized outputs. The order review keeps one business interface while dispatching only the warehouse-specific aggregation syntax; the volume reviews apply one feed inventory and an explicit minute-level policy across both platform inputs. The scheduled review first derives the complete active window set from that policy, then reports each eligible raw-to-staged row-count delta. |
