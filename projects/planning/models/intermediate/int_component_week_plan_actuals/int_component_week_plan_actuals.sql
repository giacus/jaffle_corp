...
actuals as (
    select
        store_id,
        component_id,
        {{ shared.week_start('balance_date_utc') }} as actual_week_start_utc,
    ...
    from {{ ref('supply', 'fct_inventory_daily') }}
    group by 1, 2, 3
),
...
components as (
    select
        ...
        component_family,
        recipe_unit
    from {{ ref('supply', 'dim_components') }}
)

select
    {{ shared.scenario_component_store_week_key('plans.scenario_name', 'plans.component_id', 'plans.store_id', 'plans.plan_week_start_utc') }} as component_week_plan_variance_key,
    plans.plan_id,
    plans.store_id,
    plans.component_id,
    ...
    {{ shared.forecast_error('coalesce(actuals.actual_used_quantity, 0)', 'plans.planned_usage_quantity') }} as usage_quantity_variance,
    {{ shared.plan_variance_status('coalesce(actuals.actual_used_quantity, 0)', 'plans.planned_usage_quantity') }} as usage_variance_status,
