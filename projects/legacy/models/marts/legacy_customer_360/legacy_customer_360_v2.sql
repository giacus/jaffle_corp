with customers as (
    select * from {{ ref('int_legacy_customer_notes') }}
),

orders as (
    select
        customer_id,
        count(*) filter (where not is_cancelled_order) as order_cnt,
        sum(case when not is_cancelled_order then order_total_major else 0 end) as gross_amt,
        sum(case when refund_event_count > 0 then 1 else 0 end) as refund_cnt,
        max(case when not is_cancelled_order then ordered_date_utc end) as last_business_dt
