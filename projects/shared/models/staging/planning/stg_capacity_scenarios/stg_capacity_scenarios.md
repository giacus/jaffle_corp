...
UTC timestamp at which the `stg_capacity_scenarios` record becomes effective.
{% enddocs %}

{% docs shared__stg_capacity_scenarios__effective_to_utc %}
UTC timestamp at which the `stg_capacity_scenarios` record stops being effective; null means open-ended.
{% enddocs %}
...
UTC timestamp when the source system last updated the `stg_capacity_scenarios` record.
{% enddocs %}
