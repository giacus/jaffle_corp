select
    {{ jaffle_shared.stable_hash(['store_id', 'operating_date']) }} as store_day_operations_key,
    store_id,
    operating_date,
    country_code,
    city,
    ...
    {{ jaffle_shared.safe_divide('ready_inside_target_count', 'order_timing_count') }} as ready_target_rate,
    planned_minutes,
    actual_minutes,
    peak_team_member_count,
    {{ jaffle_shared.safe_divide('order_count', 'actual_minutes') }} as orders_per_actual_minute,
    shift_exception_count,
    quality_check_count,
    quality_exception_count,
    {{ jaffle_shared.safe_divide('quality_exception_count', 'quality_check_count') }} as quality_exception_rate,
    component_day_count,
    supply_risk_component_count,
    {{ jaffle_shared.safe_divide('supply_risk_component_count', 'component_day_count') }} as supply_risk_component_rate,
    incident_count,
